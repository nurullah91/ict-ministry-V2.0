      ...
      <div className="w-2/3 lg:w-full bg-green-200 mb-6">
        ...
        >
          ...
          {navItems.map((item, index) => (
            <ul
              key={index}
              onMouseEnter={() => setOpenedMenu(item.position)}
              onMouseLeave={() => setOpenedMenu("")}
              className="group relative nav-title"
            >
              ...
              {/* Sub menu */}
              {openedMenu === item.position && (
                <div data-aos="fade-up" data-aos-duration="300">
                  <div
                    className={`absolute lg:top-7 top-0 z-20 left-28 w-[250px] shadow-lg bg-red-500/20 backdrop-blur-md grid grid-cols-1 
                ${
